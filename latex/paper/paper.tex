...
A common solution in the literature is to remove control units from the estimated sample that are most likely to be affected by spillovers. I do this in Figure \ref{fig:bias_as_treat_prob} and the results are shown by the line with circle markers. The line stays centered constantly around zero as all control units that no longer followed their counterfactual trend were removed. This however requires complete knowledge of which units are affected. The bias would fall between the two lines as fewer problematic control units are removed. 
